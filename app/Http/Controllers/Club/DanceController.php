...
{
    // ダンス TOPページ (Discussページ)-----------------------------------------------------------------------
    public function dance() {
        $auths        = Auth::user();
        $alls         = Club::where('club_id',3)->latest()->get();
        $comments     = Clubchat::where('club_id',3)->latest()->get();
        $members      = ClubId::where('club_id',3)->latest()->get();
        $rules        = Club::orderBy('id','asc')->paginate(3);
        $titles       = Club::where('user_id',Auth::user()->id)->get();
        $users        = DB::table('club_ids')->where('club_id',3)->distinct()->count('user_id');
        
        $main_manager = Admin::where('admin', 1)->where('club_id',3)->latest()->first();
        $sub_manager  = Admin::where('admin', 2)->where('club_id',3)->latest()->first();
    ...
    public function member() {
        ...
        return view('club/dance/dancemember', [
            'alls'   => $alls,
            'auths'  => $auths,
            'users'  => $users,
    ...
    public function discussion_edit(Club $alls) {
        ...
        return view('club/dance/dancediscussionedit', ['all' => $alls]);
    }
    // 編集処理--------------------------
    ...
    public function rule() {
        $alls   = ClubId::where('club_id',3)->latest()->get();
        $auths  = Auth::user();
        $admins = Admin::where('user_id',Auth::user()->id)->where('club_id',3)->latest()->get();
        $users  = DB::table('club_ids')->where('club_id',3)->distinct()->count('user_id');
        $rules  = Rule::where('club_id',3)->orderBy('id', 'asc')->get();
        
        $main_manager = Admin::where('admin', 1)->where('club_id',3)->latest()->first();
        $sub_manager  = Admin::where('admin', 2)->where('club_id',3)->latest()->first();
       
        return view('club/dance/dancerule', [
            'admins' => $admins,
            'auths'  => $auths,
            'users'  => $users,
            'alls'   => $alls,
    ...
    public function rule_store(Request $request) {
        ...
        $rules->save();    
        return redirect('/dance_rule');
    }
    // 編集画面--------
    public function rule_edit(Rule $rules) {
        $all = Rule::where('id',$rules);
        return view('club/dance/danceruleedit', ['rule' => $rules]);
    }
    // 編集処理--------
    ...
    public function admin() {
        $alls         = ClubId::where('club_id',3)->latest()->paginate(7);
        $auths        = Auth::user();
        ...
        return view('club/dance/danceadmin', [
            'alls'      => $alls,
            'auths'     => $auths,
            'club_ids'  => $club_ids,
            'rules'     => $rules,
    ...
    public function admin_store(Request $request) {
        ...
        $admins->save();    
        return redirect('/dance_admin');
    }
    // EVENTページ表示 ----------------------------------------------------------------------------------------------------------------
    public function event() {
        $auths          = Auth::user();
        $alls           = ClubId::where('club_id',3)->latest()->get();
        $all_events     = Event::where('club_id',3)->latest()->get();
        $participates   = 
        
        
        
        $users          = DB::table('club_ids')->where('club_id',3)->distinct()->count('user_id');
        $event_comments = EventComment::where('club_id',3)->latest()->get();
        ...
        return view('club/dance/danceevent', [
            'auths'          => $auths,
            'alls'           => $alls,
    ...
    public function event_store(Request $request) {
        ...
        $events->save();    
        return redirect('/dance_event');
    }
    // 編集画面-----------------------------------------
    public function event_edit(Event $all_events) {
        $all = Event::where('id',$all_events);
        return view('club/dance/danceeventedit', ['all_event' => $all_events]);
    }
    // 編集処理-------------------------
    ...
    public function participants() {
        ...
        return view('club/dance/danceparticipants', [
            'alls'          => $alls, 
            'auths'         => $auths,
            'users'         => $users,
            'go_sum'        => $go_sum,
