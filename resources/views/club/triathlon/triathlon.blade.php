...
{{ csrf_field() }}
    ...
    <div class="panel panel-default">  
        <form  enctype="multipart/form-data" action="{{ url('triathlondiscuss') }}" method="POST">
            {{ csrf_field() }}
    ...
    @foreach ($alls as $all)
        <div class="panel panel-default">  
            ...
            <div class="panel-body">
                ...
                @if ($all->user_id == $auths->id)
                    <div class="discussion_editdelete">
                        ...
                        <!-- 削除ボタン -->
                        <div>
                            <form action="{{ url('triathlon_discussions/'.$all->id) }}" method="POST">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
            ...
            <div class="panel-footer">    
                <!--<form action="{{ url('triathlon_discussion_comment') }}" method="POST">-->
                {{ csrf_field() }}
                
                <div id="clubchat">
                    <form  @submit.prevent="discussion()">
                        <div class="chat_wrapper">
                            <!--<input type="hidden" id="user_id" value="{{ $auths->id }}">-->
                            <input type="text"   id="body2"    name="body" id="body" placeholder=" Add a comment..." v-model="body2">
                            <input type="hidden" id="post_id" name="post_id" value="{{ $all->id }}">
                            <input type="hidden" id="club_id" name="club_id" value="1">
                        </div>
                    </form>
                </div>
...
<script src="{{ asset('js/clubchat.js') }}"></script>
<script>
// --vue.jsにセッション経由でuser_idを渡す処理----------------
    ...
    window.sessionStorage.setItem("post_id", postId);
    
    const clubId = document.getElementById("club_id").value;
    window.sessionStorage.setItem("club_id", clubId);
</script>
