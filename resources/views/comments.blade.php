...
<div class="panel-body">
    ...
    <div>
        <div class="commitment_title">
            <a href="{{ action('IntroductionsController@term') }}" class="commitment_text">コミットメント　</a>
        </div>
        ...
        <div>
            ...
            @if (count($comment1) > 0)
                <div class="comment123">
                    ...
                    <div class="comment_text">
                        {{ str_limit($comment1->comment, $limit = 40, $end = '....') }}
            ...
            @if (count($comment2) > 0)    
                <div class="comment123">
                    ...
                    <div class="comment_text">
                        {{ str_limit($comment2->comment, $limit = 40, $end = '....') }}
            ...
            @if (count($comment3) > 0)     
                <div class="comment123">
                    ...
                    <div class="comment_text">
                        {{ str_limit($comment3->comment, $limit = 40, $end = '....') }}
    ...
    <div class="icon_bgc">
        <div class="icon_wrapper">
            @if (count($profiles) > 0)
                <a href="{{ action('ProfilesController@index') }}">
                    <img src="upload/{{ $profiles->comment_img }}"  class="icon_img">
                </a>
            @else
                <a href="{{ action('ProfilesController@index') }}">
            ...
            @endif
            <!-- 名前/一言 -->
            <a href="{{ action('LikesController@index') }}" class="oneword_text">
                <!--名前--->
                <h4 class="icon_name">{{ $auths->name }}</h4>
                <!--一言--->
                @if ( $oneword )
                    <p>{{ $oneword->oneword }}</p>
                @else
    ...
    <div class="panel panel-default">
        <div class="activity_title">
            <h4><a href="{{ action('ActivitiesController@graph') }}"class="activity_text">活動実績　</a></h4>
        </div>
                    ...
                    @foreach ($activities as $activity)
                        <div class="unit">
                            ...
                            <div class="activity_record_text">
                                ...
                                <!--タイトル--->
                                <div><a href="{{ action('ActivitiesController@edit',$activity) }}" class="activity_record_title">{{ str_limit($activity->act_title, $limit = 38, $end = '....') }}</a></div>
                                <!-- 競技 ----->
            ...
            <div class="goal_dream_title">
                ...
                {{ csrf_field() }}
                    ...
                    <div class="goal_input">
                        <input type="text" name="goal" id="goal" placeholder=' 目標/夢を入力する' value="{{ old('goal') }}">
                        ...
                        @endif
                        <!-- 登録ボタン -->
                        <button type="submit" class="btn btn-default goal_btn"><div>+</div></button>
                    </div>
                </form>
                <!-- 登録フォーム END---------->
            </div>
                     ...
                     <div class="unit_goal"> 
                        <div> 
                             <div class="activity_record_text">
                                ...
                                @else
                                    <p class="goal_record_date">【達成日】20YY/MM/DD</p>
                                @endif
                                
                                <p class="card-text"><a href="{{ action('GoalsController@edit',$goal) }}" class="goal_record_title">{{ str_limit($goal->goal, $limit = 22, $end = '....') }}</a></p>
